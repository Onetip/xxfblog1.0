...
<body>
    ...
    <div class="page-header clearfix">
        ...
        <ul class="nav nav-tabs">
            <li class="active">
                <a href="<?php echo U('Background/index');?>">图片列表</a>
            </li>
            <li>
                <a href="<?php echo U('Background/add');?>">添加图片</a>
            </li>
        </ul>
    </div>
...
<!--提示 end-->
    ...
    <div class="content-box-content">
        ...
        <form action="<?php echo U('Background/remove');?>" method="post" class="batch-form">
            ...
            <table class="table table-striped table-framed table-hover">
                ...
                <?php if(is_array($list)): $i = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$row): $mod = ($i % 2 );++$i;?><tr>
                        ...
                        <td>
                            <div class="preview">
                                <img src="/myblog/Uploads<?php echo ($row['img']); ?>">
                            </div>
                        </td>
                        <td><?php echo ($row['img']); ?></td>
                        <td>
                            <span class="fa fa-eye on-off fa-black" <?php if($row['status'] == 0): ?>style="display:none;"<?php endif; ?>></span>
                            <input type="hidden" value="Background|status|<?php echo ($row['id']); ?>|1">
                            <span class="fa fa-eye-slash on-off fa-red" <?php if($row['status'] == 1): ?>style="display:none;"<?php endif; ?>></span>
                            <input type="hidden" value="Background|status|<?php echo ($row['id']); ?>|0">
                        </td>
                        <td>
                            <a href="<?php echo U('Background/update',array('model'=>'Background','alias'=>'b','id'=>$row['id']));?>" title="编辑" class="modify">
                                <span class="fa fa-pencil"></span>
                            </a>&nbsp;
                            <a href="#" title="删除" class="delete-delete">
                                <span class="fa fa-remove"></span>
                            </a><input type="hidden" value="<?php echo U('Background/remove',array('model'=>'Background','id'=>$row['id']));?>">
                        </td>
